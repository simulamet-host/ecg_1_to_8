...
def get_model_state(type,version,input_folder):
    ...
    print(f"Downloading model state dict: {type}_{version}")
...
def run_inference(type,version,input_path,output_path,normalize,unit):
    #############################################
    ...
    print(f"Data normalized,unit specified: {unit}")
...
def Syn_trans(type="norm",version="best",input_path=None,output_path=None,normalize=True,unit="mV"):
    ...
    Choose type of model that fits best: "syn","norm","patho", the best version is already set.
    ...
    """
    run_inference(type,version,input_path,output_path,normalize,unit)


#Syn_trans(type="norm",input_path="C:/Users/tobia/Desktop/Simula/ecg_gen/input",normalize=True,unit="mV")
